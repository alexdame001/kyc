...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2>BILLING Dashboard - Pending KYC Requests</h2>
        ...
        <a href="{{ route('billing.reports') }}" class="btn btn-info btn-lg">View Reports</a>
    </div>
    ...
    <form method="GET" class="mb-4 d-flex gap-2">
        ...
        @if(request('search'))
            <a href="{{ route('billing.dashboard') }}" class="btn btn-secondary">Clear</a>
        @endif
    </form>
            ...
            <tbody>
                @forelse($kycForms as $form)
                    <tr>
                        ...
                        <td><span class="badge bg-primary">{{ $form->buname ?? 'N/A' }}</span></td>
                        ...
                        <td>
                            <span class="badge bg-warning text-dark">
                                {{ ucfirst($form->billing_status ?? 'pending') }}
                            </span>
                        </td>
...
<script>
    ...
    function singleAction(id, status) {
        ...
        if (status === 'rejected' && !notes?.trim()) return alert('Reason is required for rejection.');

        fetch(`/billing/update/${id}`, {
            method: 'POST',
            headers: {
